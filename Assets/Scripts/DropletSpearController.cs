...
public class DropletSpearController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        transform.localScale = new Vector3(1.5f, 2f);
    }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (info.ignoreType != null)
        {
            ...
            {
                if (!((other.tag == "Locked Fence" && !other.GetComponent<LockedFenceController>().locked) || other.tag == "Enemy"))
                    GameObject.Destroy(this.gameObject);
                if (other.tag == "Player" || other.tag == "Enemy")
                    other.gameObject.GetComponent<HealthController>().LoseHealth(30f);
            }
